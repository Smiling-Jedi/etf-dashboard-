"""
ETF三因子轮动策略 - 周度评估+1.5倍阈值版
验证：周频+1.5倍阈值，周五收盘排名，下周一开盘买入
"""
...
def run_weekly_backtest_with_threshold():
    """运行周度回测（带1.5倍阈值）"""
    print("=" * 60)
    print("ETF三因子轮动策略 - 周度评估+1.5倍阈值版")
    print("=" * 60)
    print(f"\n回测区间: {START_DATE} ~ {END_DATE}")
    print(f"佣金费率: {COMMISSION_RATE*10000:.0f} bps")
    print(f"调仓规则: 周五收盘排名，1.5倍阈值，下周一开盘买入")
    ...
    for friday in all_fridays:
        ...
        if should_trade and target != holding and target is not None:
            ...
            if monday_open is not None:
                ...
                print(f"  {monday_date.strftime('%Y-%m-%d')} 调仓 → {ETF_POOL[target]} (得分: {current_score:.3f})")
    ...
    axes[0].plot(nav_df.index, nav_df['nav'], label=f'周度+1.5倍阈值 (年化{annual_return:.1f}%)', color='#D62828', linewidth=2)
    axes[0].axhline(y=17.20, color='green', linestyle='--', label='目标: 17.20倍')
    axes[0].set_title(f'周度+1.5倍阈值: 净值 {final_nav:.2f}倍 | 年化 {annual_return:.1f}% | 夏普 {sharpe:.2f}', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('backtest_weekly_with_threshold.png', dpi=150, bbox_inches='tight')
    print(f"\n结果图已保存: backtest_weekly_with_threshold.png")

    return nav_df, trade_log


if __name__ == '__main__':
    nav_df, trade_log = run_weekly_backtest_with_threshold()
