"""
ETF三因子动量轮动策略回测 - 原文组合验证版
使用论坛原文的ETF组合：512890 + 159949 + 513100 + 518880
验证是否能复现原文结果（7年14.71倍，年化45.4%）
"""
...
# ============ 策略参数配置（与原文完全一致）============
ETF_POOL = {
    '512890.SH': '红利低波ETF',      # A股价值 - 与原文一致
    '159949.SZ': '创业板50ETF',      # A股成长 - 与原文一致
    '513100.SH': '纳指ETF',          # 海外 - 与原文一致
    '518880.SH': '黄金ETF'           # 避险 - 与原文一致
}
...
# 因子权重
WEIGHT_BIAS = 0.30       # 乖离动量权重
WEIGHT_SLOPE = 0.30      # 斜率动量权重
WEIGHT_EFFICIENCY = 0.40 # 效率动量权重

# 调仓阈值
SWITCH_THRESHOLD = 2.0  # 【优化】从1.5提高到2.0，降低交易频率

# 交易费率 (单边0.03%)
COMMISSION_RATE = 0.0003

# 回测参数
START_DATE = '2019-01-01'  # 与原文一致
END_DATE = '2026-03-26'
...
def run_backtest():
    ...
    print(f"策略参数: BIAS_N={BIAS_N}, MOMENTUM_DAY={MOMENTUM_DAY}, 阈值={SWITCH_THRESHOLD}")
    ...
    print(f"{'夏普比率':<20} {orig_sharpe:<15.2f} {sharpe:<15.2f} {sharpe - orig_sharpe:+.2f}")
    ...
    plt.tight_layout()
    plt.savefig('backtest_result_original.png', dpi=150, bbox_inches='tight')
    print(f"\n图表已保存: backtest_result_original.png")

    # ============ 保存Excel ============
    with pd.ExcelWriter('backtest_result_original.xlsx', engine='openpyxl') as writer:
    ...
    print(f"数据已保存: backtest_result_original.xlsx")
    print(f"\n{'='*70}")
    print("回测完成！")
    print(f"{'='*70}")
