...
def run_daily_backtest():
    """运行日频回测（原文策略）"""
    print("=" * 60)
    print("ETF三因子轮动策略 - 原文版本（日频+1.5倍阈值）")
    print("=" * 60)
    print(f"\n回测区间: {START_DATE} ~ {END_DATE}")
    print(f"佣金费率: {COMMISSION_RATE*10000:.0f} bps")
    print(f"调仓规则: 日频评估，次日开盘买入，1.5倍阈值")
    ...
    for i, date in enumerate(trade_dates):
        ...
        # 选择目标ETF
        target = select_best_etf_with_threshold(factors, holding)
        ...
        if target != holding and target is not None:
            ...
            if i + 1 < len(trade_dates):
                ...
                if target in etf_data:
                    buy_price = etf_data[target].loc[next_date, 'open']
    ...
    print("与目标数据对比")
    print("=" * 60)
    print(f"日度目标净值: 14.24倍 | 实际: {final_nav:.2f}倍 | 差异: {final_nav-14.24:+.2f}")
    print(f"日度目标年化: 44.73% | 实际: {annual_return:.2f}% | 差异: {annual_return-44.73:+.2f}%")
    print(f"日度目标交易: 111次   | 实际: {trade_count}次")
    print(f"\n周度目标净值: 17.20倍")
    print(f"周度目标年化: 48.59%")

    return nav_df, trade_count


if __name__ == '__main__':
    nav_df, trade_count = run_daily_backtest()
