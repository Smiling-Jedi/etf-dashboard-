"""
ETF三因子动量轮动策略回测 - 月度评估版
每月最后一个交易日计算因子，次月第一个交易日调仓
"""
...
def get_monthly_trade_dates(trade_dates):
    """获取每月的最后一个交易日"""
    df = pd.DataFrame({'date': trade_dates})
    df['year_month'] = df['date'].dt.strftime('%Y-%m')
    # 每月最后一个交易日
    monthly_dates = df.groupby('year_month')['date'].max().tolist()
    return sorted(monthly_dates)


def run_backtest_monthly():
    print("=" * 70)
    print("ETF三因子动量轮动策略回测 - 月度评估版")
    print("=" * 70)
    print(f"\n评估频率: 每月最后一个交易日（次月第一个交易日执行）")
    ...
    # 获取每月的评估日（月末）
    monthly_dates = get_monthly_trade_dates(all_trade_dates)
    print(f"\n总交易日: {len(all_trade_dates)} 天")
    print(f"月度评估次数: {len(monthly_dates)} 次")
    ...
    for i, date in enumerate(all_trade_dates):
        # 只在每月最后一个交易日评估
        is_monthly_eval = date in monthly_dates
    ...
    # 保存
    with pd.ExcelWriter('backtest_result_monthly.xlsx', engine='openpyxl') as writer:
    ...
    print(f"\n数据已保存: backtest_result_monthly.xlsx")
